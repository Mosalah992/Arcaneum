-- One volume added after the corpus was written: The Black Arts on Trial,
-- supplied by the College and listed on its register at R5.
--
-- WRITTEN BY HAND, NOT GENERATED. The shelf migrations 0002-0012 are numbered
-- in one pass and every id in them is positional, so slotting a book into the
-- middle of that sequence would renumber every row after it and rewrite every
-- migration that carries them. It gets the next free id instead and the
-- catalogue orders by call number rather than by id — see
-- functions/api/tomes/index.ts.
--
-- Idempotent on the call number, so it survives a full reseed in either order:
-- if a regenerated 0006 already carries this book, the DELETE takes that row
-- out and this one replaces it.

DELETE FROM tomes WHERE call_number = 'AR-V-051';

INSERT INTO tomes (id, call_number, title, author, school, body, restricted)
VALUES (
...
-- The search index is external-content, so a row inserted behind its back is
-- invisible to MATCH until it is told. A rebuild is the honest way to say it.
